        ...
        {items.map((application) => <article key={application.id} className="rounded-xl border bg-white p-4 shadow-sm">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            {application.status === 'en_attente' && <>
              <button disabled={busy === application.id} onClick={() => review(application.id, 'approve')} className="flex items-center gap-1 rounded-lg bg-yms-600 px-3 py-2 text-sm text-white disabled:cursor-wait disabled:opacity-60"><Check size={16} /> {busy === application.id ? 'Traitement...' : 'Valider'}</button>
              <button disabled={busy === application.id} onClick={() => review(application.id, 'reject')} className="flex items-center gap-1 rounded-lg border px-3 py-2 text-sm text-red-600 disabled:cursor-wait disabled:opacity-60"><X size={16} /> Refuser</button>
            </>}
          </div>
          {reviewError?.id === application.id && <p role="alert" className="mt-3 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700">Erreur de validation : {reviewError.message}</p>}
        </article>)}
