...
import { getCurrentProfile, updatePassword } from '../../shared/services/authService'
...
export function SetPassword({ mode }: SetPasswordProps) {
  ...
  useEffect(() => {
    ...
    getCurrentProfile().then((profile) => {
      if (!active) return
      setValidLink(Boolean(profile))
      setMessage(profile ? '' : 'Ce lien est invalide ou a expiré. Demandez un nouveau lien.')
    }).catch(() => {
      if (!active) return
      setValidLink(false)
      setMessage('Ce lien est invalide ou a expiré. Demandez un nouveau lien.')
    })
    return () => { active = false }
  }, [])
